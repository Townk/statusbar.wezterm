...
function M.create_tab_title(tab)
  ...
  if user_title ~= nil and #user_title > 0 then
    ...
    if string.sub(tab.active_pane.title, 1, 10) == 'Copy mode:' then
      in_copy_mode = true
    end
  else
    ...
    if title == '' then
      ...
      if dir:sub(1, 10) == 'Copy mode:' then
        in_copy_mode = true
        dir = dir:sub(12)
      end
  ...
  if in_copy_mode and M.status_config.key_tables.copy_mode.icon then
